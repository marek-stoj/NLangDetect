...
namespace NLangDetect.Core.Utils
{
  ...
  {
    private static readonly string _MessagesResourceName = string.Format("{0}.Messages.properties", MethodBase.GetCurrentMethod().DeclaringType.Namespace);
    ...
    private static Dictionary<string, string> LoadMessages()
    {
      Stream messagesStream =
        Assembly.GetExecutingAssembly()
          .GetManifestResourceStream(_MessagesResourceName);

      if (messagesStream == null)
      {
        throw new InternalException(string.Format("Couldn't get embedded resource named '{0}'.", _MessagesResourceName));
      }
